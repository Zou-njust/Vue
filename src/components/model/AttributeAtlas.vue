<template>
  <div>
        ...
        <el-scrollbar style="position: relative">
          <el-form ref="form" :inline="true" :model="form" class="scrollbar-flex-content">
            <el-form-item label="查询类型" class="scrollbar-demo-item1" style="margin-left: 10px">
              <el-select v-model="form.searchTypeValue" placeholder="请选择查询类型" style="width: 150px">
                <el-option v-for="(item, index) in form.searchType" :label="item" :value="item" :key="index"/>
              </el-select>
              <el-button v-show="form.searchTypeValue===''" type="primary" icon="el-icon-search"
                         style="margin-left: 20px"
                         @click="search">查看全部知识图谱
              </el-button>
            </el-form-item>
            ...
            <el-form-item v-show="form.searchTypeValue === '按节点查询'" class="scrollbar-demo-item3">
              ...
              <el-button type="primary" icon="el-icon-search" @click="search">查看全部知识图谱</el-button>
            </el-form-item>
            ...
            <el-form-item style="position: absolute;right:10px">
              <el-button @click="showImportDialog">从Mysql导入</el-button>
            </el-form-item>
          </el-form>
        </el-scrollbar>
      </div>
          ...
          >
            ...
            <el-dialog :visible.sync="insertRelVis" title="请输入添加关系的信息">
              ...
              <div slot="footer" class="dialog-footer">
                <el-button @click="insertRelVis = false">取 消</el-button>
    ...
    <el-dialog :visible.sync="importDialogVisible" title="导入Mysql数据" width="30%" :close-on-click-modal="false">
      <el-radio-group v-model="importNodeOrRel" style="margin-bottom:20px">
        <el-radio label="node">节点</el-radio>
        <el-radio label="rel">关系</el-radio>
      </el-radio-group>
      <div v-show="importNodeOrRel==='node'">
        <el-select v-model="chosenNodeTableName" placeholder="请选择要导入的表" style="margin-bottom:20px">
          <el-option v-for="(item, index) in importNodeTableOptions" :label="item" :value="item" :key="index"/>
        </el-select>
      </div>
      <div v-show="importNodeOrRel==='rel'">
        <el-select v-model="chosenRelTableName" placeholder="请选择要导入的表" style="margin-bottom:20px">
          <el-option v-for="(item, index) in importRelTableOptions" :label="item" :value="item" :key="index"/>
        </el-select>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="importDialogVisible = false;chosenNodeTableName='';chosenRelTableName=''"
                   style="margin-left:70%">取 消</el-button>
        <el-button type="primary" @click="importFromMysql" style="float: right">确 定</el-button>
      </div>
...
export default {
  ...
  data() {
    ...
    chosenRelTableName: '',
      ...
      linkTypeOptions: [],
      //导入Mysql表名
      importNodeTableOptions: [],
      importRelTableOptions: [],
      importNodeOrRel: 'node',
    }
  },
  created() {
    // 获得页面基本信息，得到全部标签
  ...
  methods: {
    ...
    showImportDialog() {
      ...
      this.$http.get('KG/getNodeTableNames').then(res => {
        if (res.data.code !== 200) return this.$message.error('获取Mysql信息失败！')
        this.importNodeTableOptions = res.data.data
      })
      this.$http.get('KG/getRelTableNames').then(res => {
        if (res.data.code !== 200) return this.$message.error('获取Mysql信息失败！')
        this.importRelTableOptions = res.data.data
      })
    },
    importFromMysql() {
      const chosenImport=this.importNodeOrRel==='node'?this.chosenNodeTableName:this.chosenRelTableName
      this.$http.get('KG/importFromMysql', {
        params: {
          tableName: chosenImport
        }
      }).then(res => {
        if (res.data.code !== 200) return this.$message.error('导入失败！')
        this.$message.success('导入成功！')
      })
      this.importDialogVisible = false
      this.chosenNodeTableName=''
      this.chosenRelTableName=''
    }
  }
}
</script>
